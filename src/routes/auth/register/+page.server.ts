...
export const actions = {
  default: async ({ request, url, cookies }) => {
    ...
    if (existing) return fail(422, { success: false, message: 'User with this email is already exists' })
    ...
    const session = await lucia.createSession(created[0].id, { email: created[0].email, username: created[0].username })
