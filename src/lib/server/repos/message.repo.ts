...
import { db } from '../db'
import { messages, type MessageDb, type MessageDbCreate, type MessageDbTree, type MessageDbDeep, type MessageId, type FIleId, messagesToFiles } from '../db/schema'

export async function getRootMessages(offset?: number, limit?: number): Promise<MessageDbDeep[]> {
  return await db.query.messages.findMany({
    offset, limit, where: isNull(messages.parentMessageId),
    with: { author: true, files: { with: { file: true } } }
  })
}

export async function getRootMessagesTrees(offset?: number, limit?: number): Promise<MessageDbTree[]> {
  const ids = await db.query.messages.findMany({
    offset, limit, where: isNull(messages.parentMessageId), columns: { id: true },
  })
