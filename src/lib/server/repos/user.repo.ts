...
export async function getUser(id: UserId) {
	return (
		(await db.query.users.findFirst({ where: eq(users.id, id), with: { oauths: true } })) || null
	)
}
export async function getUserByEmail(email: string) {
	return (
		(await db.query.users.findFirst({ where: eq(users.email, email), with: { oauths: true } })) ||
		null
	)
}
...
): Promise<UserDbDeep | null> {
	const oauth = await db.query.oauths.findFirst({
		where: and(eq(oauths.provider, provider), eq(oauths.providerUserId, providerUserId)),
		with: {
			user: {
				with: {
					oauths: true
